  ...
  test('moves', () => {
    for (const gen of gens) {
      ...
      for (const move of p) {
        ...
        if (move.id === 'dragondarts') delete (move as any).multihit;
  ...
  test('species', () => {
    for (const gen of gens) {
      ...
      for (const specie of p) {
        ...
        const s = c.get(specie.id)!;
        delete (s as any).otherFormes;
        delete (specie as any).otherFormes;
        expect(s).toEqual(specie);
      }
    }
  });
